...
## QR พร้อมเพย์ตอนเติมเงิน

`GET /api/topups/qr?amount=500` คืน payload มาตรฐาน EMV QRCPS กับรูป PNG (data URL)
ให้หน้า **กระเป๋าเงิน** เอาไปแสดง ลูกค้าสแกนจ่าย แล้วอัปโหลดสลิปใบเดิมต่อที่ฟอร์มด้านล่าง
— QR แค่ *ขอ* เงิน ตัวที่เติมเข้ากระเป๋าจริงยังเป็นการตรวจสลิปกับธนาคารเหมือนเดิม
ไม่ต้องตั้งค่า env เพิ่ม ไม่ต้องต่อผู้ให้บริการเจ้าไหน

เลขที่เอาไปทำ QR มาจาก `store_settings.topup_receiver_account` เท่านั้น ไม่รับจาก query
