  ...
  try {
    ...
    const [fulfilments, orders] = await Promise.all([
      supabase
        ...
        .select(
          'id, order_id, supplier, game_title, account_username, account_password, code_requests_used, code_requests_max, hwid_reset_count, hwid_reset_last_at, status, error_message, created_at'
        )
    ...
    const toAccount = (row: Row) => ({
      ...
      hwidResetCount: typeof row.hwid_reset_count === 'number' ? row.hwid_reset_count : 0,
      hwidResetLastAt: str(row.hwid_reset_last_at) || undefined,
