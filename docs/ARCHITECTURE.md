...
สินค้าที่ตั้ง `is_unlimited` ข้ามสองขั้นแรก (ไม่ตรวจ ไม่ตัดสต็อก) และตอนส่งมอบจะคัดลอก
รหัสใบแรกในคลังให้ตามจำนวนที่สั่งโดยไม่ตั้ง `claimed_at` — รหัสใบเดิมจึงขายต่อได้เรื่อย ๆ
`supplier_ref` ของแถวที่ส่งมอบจึงผูกกับคำสั่งซื้อ ไม่ใช่กับแถวในคลัง เพราะแถวเดิมถูกส่งซ้ำได้
